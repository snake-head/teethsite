...
import vtkRootHandleRepresentation from "./RootHandleRepresentation";
...
function vtkHandleWidget(publicAPI, model) {
    ...
    publicAPI.createDefaultRepresentation = () => {
        if (!model.widgetRep) {
            model.widgetRep = vtkRootHandleRepresentation.newInstance({
    ...
    publicAPI.moveAction = (callData) => {
        ...
        if (model.widgetState === WidgetState.START) {
            ...
            if (state !== newState) {
                model.widgetRep.highlight(model.widgetRep.computeInteractionState(position));
                publicAPI.render();
            }
            ...
            switch(currentInteractionState){
                ...
                case InteractionState.SELECTING_ACTOR_BOTTOM:
                    model.widgetRep.rotateBottom(position); // 操作底部小球，是绕顶部小球旋转
                    // model.onAfterModifyLengthChange(model.widgetRep.getCenters());
                    publicAPI.invokeInteractionEvent();
                    publicAPI.render();
                    break;
                case InteractionState.SELECTING_ACTOR_TOP:
                    model.widgetRep.rotateTop(position); // 操作顶部小球，是绕底部小球旋转
                    // model.onAfterModifyLengthChange(model.widgetRep.getCenters());
                    publicAPI.invokeInteractionEvent();
                    publicAPI.render();
                    break;
